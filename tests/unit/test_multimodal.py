...
def test_connection_scan_uses_transit_when_faster_than_direct_walk():
    value = earliest_arrival_minutes(
        ...
        connections=[{"trip_id": "t1", "from_stop": "s1", "to_stop": "s2", "departure_seconds": 8 * 3600 + 5 * 60, "arrival_seconds": 8 * 3600 + 15 * 60}],
        trip_routes={"t1": "r1"},
...
def test_connection_scan_respects_disabled_trip_without_disabling_route():
    connections = [
        {"trip_id": "t1", "from_stop": "s1", "to_stop": "s2", "departure_seconds": 8 * 3600 + 5 * 60, "arrival_seconds": 8 * 3600 + 15 * 60},
        {"trip_id": "t2", "from_stop": "s1", "to_stop": "s2", "departure_seconds": 8 * 3600 + 20 * 60, "arrival_seconds": 8 * 3600 + 30 * 60},
    ]
    common = dict(
        zone_node="zone",
        departure_seconds=8 * 3600,
        connections=connections,
        trip_routes={"t1": "r1", "t2": "r1"},
        stop_walk={"s1": {"zone": 3.0}, "s2": {"zone": 99.0}},
        facility_walk_by_node={"zone": 50.0, "n1": 50.0, "n2": 4.0},
        stop_nodes={"s1": "n1", "s2": "n2"},
    )
    assert earliest_arrival_minutes(**common) == 19.0
    assert earliest_arrival_minutes(**common, disabled_trips={"t1"}) == 34.0
...
def test_stop_transfer_edges_respect_network_threshold_and_buffer():
    ...
    transfers = stop_transfer_edges(stop_walk, stop_nodes, max_walk_minutes=10.0, transfer_buffer_minutes=1.0)
    assert transfers["s1"] == [{"to_stop": "s2", "walk_minutes": 4.5, "transfer_minutes": 5.5}]
    assert {edge["to_stop"] for edge in transfers["s2"]} == {"s1", "s3"}


def test_connection_scan_can_board_second_trip_after_walking_transfer():
    connections = [
        {"trip_id": "t1", "from_stop": "s1", "to_stop": "s2", "departure_seconds": 8 * 3600 + 5 * 60, "arrival_seconds": 8 * 3600 + 10 * 60},
        {"trip_id": "t2", "from_stop": "s3", "to_stop": "s4", "departure_seconds": 8 * 3600 + 16 * 60, "arrival_seconds": 8 * 3600 + 25 * 60},
    ]
    common = dict(
        zone_node="zone",
        departure_seconds=8 * 3600,
        ...
        stop_walk={"s1": {"zone": 2.0}, "s2": {"zone": 99.0}, "s3": {"zone": 99.0}, "s4": {"zone": 99.0}},
    ...
    with_transfer = earliest_arrival_minutes(**common, stop_transfers={"s2": [{"to_stop": "s3", "walk_minutes": 4.0, "transfer_minutes": 5.0}]})
...
def test_walking_transfer_does_not_override_disabled_second_route():
    value = earliest_arrival_minutes(
        ...
        stop_nodes={"s1": "n1", "s2": "n2", "s3": "n3", "s4": "n4"},
        disabled_routes={"r2"},
        stop_transfers={"s2": [{"to_stop": "s3", "walk_minutes": 4.0, "transfer_minutes": 5.0}]},
    )
    assert value == 60.0


def test_population_summary_uses_weighted_population():
    zones = [{"zone_id": "a", "population": 100.0}, {"zone_id": "b", "population": 50.0}]

...
| OSM Ozu A1 walking / hospitals | OpenStreetMap / Overpass | https://www.openstreetmap.org/copyright | ODbL 1.0 | CI取得時点 | GTFS stop bbox + 0.02° | Overpass JSON | WGS84 | 取得時点 | 徒歩routing、公開病院位置 | B | Raw抽出物はcommitせずCI一時入力。walking graph 38,860 nodes / 80,102 directed edges。OSM hospital 7表現のうちA1.5で公式台帳照合済み5件を採用 |
| Ehime medical basic information 2026-08-01 | 愛媛県医療対策課 | https://www.pref.ehime.jp/page/50405.html | 県公式Web掲載。CC BY再配布可能データとは扱わない | 2026-08-01 | 医療機関 | XLSX | 緯度経度列あり | 県更新時 | A1.5病院現行性の照合Gateのみ | A | CIで一時取得。Raw Excel、公式名称・住所・座標・機関コードをrepository/Pagesへ再配布しない。A1 bbox内の活動中病院5件を確認しOSM 5件と1対1照合 |
...
| R2 simplified population mesh 100m Ozu | 地域・交通データ研究所 | https://gtfs-gis.jp/teikyo/ | CC BY | 2020国勢調査250mを簡易按分 | 100m mesh | CSV / Shapefile ZIP | mesh / GIS | 公開更新時 | A1人口分母 | B | 5,478 rows。実100mセル人口ではなく簡易按分値。A1 envelopeで1,840 cellsを使用 |
...
| medical facilities | 国土数値情報 | https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-P04-v3_0.html | 国土数値情報利用規約 | 2020年度等 | 点 | GML / Shapefile / GeoJSON | JGD2011 | 更新時 | 将来の公開可能な病院GIS候補 | B | 現行性は愛媛県公式台帳等で別途照合する |
...
| Ozu Gururin GTFS | 大洲市 | https://www.city.ozu.ehime.jp/site/opendata/44871.html | CC BY 4.0 | feed 2026-04-01〜2027-03-31 / version 5.0 | 37停留所・4 routes・11 trips | GTFS ZIP | WGS84 | 市更新 | A1公共交通アクセシビリティ | A | 2026-08-22 CIで実取得・validator PASS。Raw ZIPはcommitせず公式URLから再取得 |
...
- 大洲市「ぐるりんおおず」GTFS version 5.0を実取得し、37 stops、4 routes、11 trips、2026-04-01〜2027-03-31の有効期間を確認した。
- 大洲市の令和2年簡易100m人口5,478 rowsを実取得した。
- A1.5ではGTFS停留所範囲のOSM徒歩道路とhospital 7表現をCI取得し、愛媛県公式医療機関台帳2026-08-01版に存在する5病院と1対1照合した。重複・未照合OSM表現2件は目的地から除外した。
- A1.5公式ExcelはCIで一時取得するだけで、raw workbookおよび公式属性はGitHub/Pages artifactへ保存しない。
- GTFS Data Japan APIは基準日時点で愛媛・伊予鉄・松山の検索結果が0件だった。これは全提供元の不存在を意味しないため、未取得の外部入力として扱う。

## 未取得データの扱い

未取得の国勢調査OD、経済センサス、物流センサス、避難所座標、災害拠点病院の現行指定情報を、存在するデータとして補完しない。公開版の入力状態はmanifestで unavailable または external_input と表示する。
